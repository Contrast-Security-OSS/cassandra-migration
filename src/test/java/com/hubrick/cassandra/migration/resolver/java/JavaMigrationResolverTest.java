...
import com.hubrick.cassandra.migration.resolver.java.dummy.V2__InterfaceBasedMigration;
import com.hubrick.cassandra.migration.resolver.java.dummy.Version3dot5;
...
public class JavaMigrationResolverTest {
    ...
    public void broken() {
        new JavaMigrationResolver(Thread.currentThread().getContextClassLoader(), new ScriptsLocation("com/hubrick/cassandra/migration/resolver/java/error")).resolveMigrations();
    }

    @Test
    public void resolveMigrations() {
        JavaMigrationResolver jdbcMigrationResolver =
                new JavaMigrationResolver(Thread.currentThread().getContextClassLoader(), new ScriptsLocation("com/hubrick/cassandra/migration/resolver/java/dummy"));
