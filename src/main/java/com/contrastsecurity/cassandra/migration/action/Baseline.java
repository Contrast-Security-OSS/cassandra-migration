...
/**
 * Created by amahakode on 16/12/15.
 */
    ...
    public void run() {
        ...
        if(schemaVersionDao.hasBaselineMarker()){
            if(! baselineMigration.getVersion().equals(baselineVersion) ||
                ...
                throw  new CassandraMigrationException("Unable to baseline metadata table " + schemaVersionDao.getTableName() + " with ("
                        + baselineVersion + "," + baselineDescription
                        + ") as it has already been initialized with ("
                        + baselineMigration.getVersion() + "," + baselineMigration.getDescription() + ")");
            }
        }else {
            if (baselineVersion.equals(MigrationVersion.fromVersion("0"))) {
                throw new CassandraMigrationException("Unable to baseline metadata table " + schemaVersionDao.getTableName() + " with version 0 as this version was used for schema creation");
            }
